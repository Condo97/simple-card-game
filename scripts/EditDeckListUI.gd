class_name EditDeckListUI extends Control

signal on_close

const DECK_BUILDER_UI_SCENE_PATH = "res://scenes/DeckBuilderUI.tscn"

@onready var deck_list_ui: DeckListUI = $View/MarginContainer/VBoxContainer/DeckListUI
...
func _ready() -> void:
	...
	deck_list_ui.on_tap_deck.connect(_on_tap_deck)
...
func _add_deck_ui_to_list(deck_model: DeckModel):
	deck_list_ui.add_deck(deck_model)

func _clear_deck_ui_list():
	deck_list_ui.clear_decks()

func _on_tap_deck(deck_ui: DeckUI):
	_show_deck_builder(deck_ui.deck_model)
...
func _show_deck_builder(deck_model: DeckModel):
